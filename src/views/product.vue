<template>
          ...
          <ol role="list" class="flex items-center space-x-4">
            <li v-for="breadcrumb in product.breadcrumbs" :key="breadcrumb.id">
              <div class="flex items-center">
                <router-link :to="breadcrumb.href" class="mr-4 text-sm font-medium text-gray-900">{{ breadcrumb.name }}</router-link>
            ...
            <li class="text-sm"> 
              <router-link :to="product.href" aria-current="page" class="font-medium text-gray-500 hover:text-gray-600">{{ product.name }}</router-link>
            </li>
          </ol>
        </nav>
        <div class="mx-auto mt-8 max-w-2xl px-4 sm:px-6 lg:max-w-7xl lg:px-8">
          <div class="lg:grid lg:auto-rows-min lg:grid-cols-12 lg:gap-x-8">
            <div class="lg:col-span-5 lg:col-start-8">
              ...
              <div class="flex justify-between">
                <span class="prose prose-sm mt-4 text-gray-500" >{{ product.model }}</span>
              </div>
            </div>
  ...
  const product = {
    name: 'Nike Dunk Low',
    model: 'Pink Velvet (GS)',
    price: '$139',
    rating: 3.9,
    reviewCount: 512,
    href: '#',
    breadcrumbs: [
      { id: 1, name: 'Skeaners', href: '/products' },
    ],
    images: [
      {
        id: 3,
